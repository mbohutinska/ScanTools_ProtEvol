...
with open(args.i) as inf:
    for j, line in enumerate(inf):
        cols = line.strip("\n").strip("\t").split("\t")
        pop, ploidy, scaff, pos, an, dp, alcode, ann = cols[:8]
        ...
        if pos in key_list[scaff - 1]:
            for geno in cols[9:]:
                if geno != "-9":
                    ...
                    gt += "-9\t"
            aas = "".join(list(cols[8])[-3:]) + "".join(map(str,[int(s) for s in list(cols[8]) if s.isdigit()])) + "".join(list(cols[8])[0:3])
            gt.strip("\t")
            newline = pop + "\t" + str(ploidy) + "\tscaffold_" + str(scaff) + "\t" + pos + "\t" + an + "\t" + dp + "\t" + alcode + "\t" + ann + "\t" + aas + "\t" + gt + "\n"
            new.write(newline)
        else:
            new.write(line)
