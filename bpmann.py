...
def calcBPM(input_file, output, outname, window_size, minimum_snps, num_pops):
    ...
    def NestedAnova(locus_list):
        ...
        for pop_site in locus: ##for one site and one pop
            ...
            nnn = float(len(pop_site[9:]))  # sample size for thispopulation
            an = ploidy * nnn  # Calculation an for the population
            ac = sum([float(geno) for geno in pop_site[9:]])  # Calculate ac for a population ##needed
        ...
        return rnum, rden, fnum, fden, Nhudson, Dhudson, Nnei, Dnei, poly, an_m, ac_m
    ...
    def calcDxy(locus_list):
        ...
        for i, pop_site in enumerate(locus):
            ploidy = float(pop_site[1])
            nnn = float(len(pop_site[9:]))
            an = ploidy * nnn
            ac = sum([float(geno) for geno in pop_site[9:]])
            if i == 0:
                p1 = (ac / an)
            if i == 1:
                p2 = (ac / an)
    ...
    out1 = open(outfile, 'w')
    out1.write("outname\tscaff\tAN0\tAN1\tAC0\tAC1\tALcode\tAAS\tann\tstart\tend\twin_size\tnum_sites\tnum_snps\tRho\tFstWC\tdxy\tAFD\tFixedDiff\tFstH\tFstN\n")
        ...
        for i, line in enumerate(data):
            ...
            if pos > start and pos <= end and scaff == oldscaff:
                if pos == old_pos:  # Accruing information from multiple populations but same locus
                    Locus.append(line)
                    alcode, ann, aas = Locus[0][6:9]

                elif len(Locus) == num_pops:  # Within current window but have moved on from previous locus
                    rnum, rden, fnum, fden, Nhudson, Dhudson, Nnei, Dnei, poly, an_m, ac_m = NestedAnova(Locus)
                    alcode, ann, aas = Locus[0][6:9]
                    if poly is True:
                        if num_pops == 2:
                            d, a = calcDxy(Locus)
                ...
                if snp_count >= minimum_snps:  # Report or exclude window
                    ...
                    try:
                        ...
                        if num_pops == 2:
                            ...
                            dn = "-9"
                            afd = "-9"
                        out1.write(outname + '\t' + scaff + '\t'+ str(an_m[0]) + '\t'+ str(an_m[1]) +'\t'+ str(ac_m[0]) + '\t'+ str(ac_m[1]) +'\t' + alcode + '\t' + aas + '\t' + ann + '\t' +  # Write calculations to file
                                   str(start) + '\t' +
                                   str(end) + '\t' +
                                   str(window_size) + '\t' +
                                   str(site_count) + '\t' +
...
if __name__ == '__main__':  # Used to run code from command line
    ...
    parser.add_argument('-i', type=str, metavar='input_file', required=True, help='input file created with recode012ann.py')
