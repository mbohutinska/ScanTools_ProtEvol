    ...
    for i, line in enumerate(table):
        ...
        if line[0].split("_")[0] == 'scaffold':
            if get_ploidy is True:
                ploidy = float(max([len(x.split("/")) for x in line[6:]]))
                get_ploidy = False
            ref = line[2]
            scaff = line[0]
            pos = line[1]
            an = line[3]
            dp = int(line[4])
            alcode = line[5].split("|")[3]
            aas = line[5].split("|")[10].split(".")[-1]
            ann = line[5].split("|")[1]
            GT = args.pop + '\t' + str(ploidy) + '\t' + scaff + '\t' + pos + '\t' + an + '\t' + str(dp) + '\t' + alcode + '\t' + ann + '\t' + aas +'\t'
            numobs = 0
            for j, gt in enumerate(line[6:]):
                gt = gt.split("/")
                if gt[0] == '.':
                    GT += '-9\t'
                else:
